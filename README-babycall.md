...
場所から同じ赤ちゃん端末に呼びかけられます。

1. すでにセットアップ済みの保護者アプリのホーム画面で **「家族を招待する」** を
   タップすると、6桁の招待コードが表示されます(有効期限10分・1回限り)。
2. 招待したい相手(祖父母・叔父さんなど)に、その番号を伝えます
   (電話・メッセージアプリなどお好きな方法で)。
3. 相手はBabyCallアプリをインストールし、**「招待コードで家族に参加する」** を選んで
   名前と番号を入力するだけで、自分の端末からも同じ赤ちゃん端末に呼びかけられる
   ようになります(自分で家族を新規作成する必要はありません)。

この「招待」の仕組みはクラウドモード専用です(ローカルモードは同じWi-Fi内の
保護者1台・赤ちゃん1台の組み合わせのみを想定した設計のため、離れた場所からの
参加はできません)。招待した人数分だけ保護者役の端末が増える形になり、誰でも
設定画面からPINの変更や自動応答のON/OFFを行えます(家族内の信頼関係を前提とした
設計で、招待された側だけ権限を制限する仕組みは現時点ではありません)。

複数人がほぼ同時に呼びかけようとした場合は、後から呼びかけた側に「ただいま他の方が
通話中です」と表示され、上書きされることはありません。
